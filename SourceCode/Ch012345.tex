...
`$a<ru$' cannot be `$a\leqslant ru$' with ``$0<u<s,\,\forall s\notin D$'', becs here $rs<ru\Rightarrow\exists\,a=rs.$\vspace{2pt}\par
\BulletPointX Note that $\Bra{u:0<u\leqslant s,\,\forall s\notin D}\supsetneq D^+.$
\SepLine

\BulletPointX Consider $-C$ and $D$ both posi. \;Omitting $C=\tilde{0},$ \,a countexa for the stmt: $C$ not posi $\Rightarrow{-C}$ posi.\vspace{1pt}\TextB{}
$CD=-\Sbra{\Par{{-C}}D}=-\Bra{a:a\leqslant cd,\,c\in\Par{{-C}}{^+},\,d\in D^+}$\TextB{}
\Blind{$CD$}${}=\Bra{{a}:a<-cd,\,\forall d\in D^+,\,\forall c\text{\;suth\;}$\uline{$0<c<-c',\,\exists\,c'$}${}\not\in C}$\hfill{\FontSmall\BigPar{$c'd<{}$\uline{$-cd<0$}}}\vspace{2pt}\TextB{}
...
$\Blind{CD}\overset{*}{=}\Bra{a:a\leqslant c'd,\;$\uline{$\forall c'\in C^-,$}$\,\forall d\in D^+}=\Bra{a:a\leqslant rd,\;\forall r\in C^-,\,\forall d\in D^+}.$\vspace{4pt}\TextB{}
We show \Par{$*$}: Supp $a<-cd,\forall c,d$ \Par{$\Rightarrow a<0$}. Asum \,$a>c'd,\,\exists\,c'\!,\:\!d.$\TextB{}
Let $-c=a\big/d\Rightarrow 0>-cd=a>c'd,$ ctradic. \;The other inclusion is obvious.\vspace{-2pt}
\SepLine
...
Supp $a\leqslant rd,\,\forall r,d.$ \,Asum $a\geqslant r'u,\,\exists\,r'\!,\:\!u.$ Now $r'u\leqslant a\leqslant rd,\,\forall r,d\Rightarrow u\geqslant d.$ ???\vspace{-2pt}
\SepLine
