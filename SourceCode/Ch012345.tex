...
Immed, $LHS\subseteq RHS.$ \,For $RHS\subseteq LHS,\,$ if we cannot fix `$\min\Qbb\Backslash D$' by a certain scalar $s_M,$ then done.\TextB{}
Othws, becs \,$s_M=\max D,$ now ``$a<rs_M,\,\forall r\in C^-$''.\TextB{}
Note that if $0<d_1<\cdots<d_n<\cdots<d_{\infty}\in D^+$ \Sbra{$d_n<s_M$} \,and $0>r_{\:\!\!1}>\cdots>r_{\:\!\!m}>\cdots>r_{\:\!\!\infty}\in C^-,$\TextB{}
\NOTICE that $d_{\infty}$ is an ordered pair $\Par{d_n,d_{n+1}}$ with $d_n<d_{n+1}.$ Simlr, $\Par{r_m,r_{m+1}}$ with $r_{m+1}<r_m.$\TextB{}
We show ``$a<r_{\:\!\!m}\,d_{n}$'' is equiv to ``$a<r_{\:\!\!m}\,s_M$''. Becs \;\!$r_{\:\!\!m+1}\,s_M<r_{\:\!\!m}\,s_M<r_{\:\!\!m}\,d_{n+1}<r_{\:\!\!m}\,d_n>r_{\:\!\!m+1}\,d_{n+1}.$\TextB{}
Asum $r_{\:\!\!m}\,s_M\leqslant t<r_{\:\!\!m}\,d_n,\,\forall m,n\in\Nbp.$ Let $u=t\big/r_{\:\!\!m}\Rightarrow s_M\geqslant u>d_n.$ Ctradic.\PfEnd
\SepLine
...
\Blind{$CD$}${}=\Bra{{a}:a<-cd,\,\forall d\in D^+,\,\forall c\text{\;suth\;}$\uline{$0<c<-r,\,\exists\,r$}${}\in C^-}$\hfill{\FontSmall\BigPar{$rd<{}$\uline{$-cd<0$}}}
...
%%%%%%%%%
%$\Blind{CD}=\Bra{a:a<rd,\;$\uline{$\forall r\in C^-,$}$\,\forall d\in D^+}.$%\vspace{4pt}\TextB{}
%%%%%%%%%
%Intuitively, \,$0<d_1<\cdots<d_n<\sup D,$ and $\sup C\leqslant r_{\:\!\!m}<\cdots<r_{\:\!\!1}<0.$ \,Becs $a<r_{\:\!\!j}\,d_k,\,\forall j,k.$\TextB{}
%又 $r_{\:\!\!j+1}\,d_k<r_{\:\!\!j}\,d_k<r_{\:\!\!j}\,d_{k-1}\Rightarrow a<r_{\:\!\!m}\,d_n<r_{\:\!\!j}\,d_k.$ \;Thus `$a<rd$' cannot be `$a\leqslant rd$'.%, becs \,$r_{\:\!\!m}\,d_{n+1}<r_{\:\!\!m}\,d_n$
%\vspace{3pt}\TextB{}
