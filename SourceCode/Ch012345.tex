...
\Anchor{0C5}\ProblemN{5}{
	\TextA{Supp $a_1,a_2,\cdots\in\Qbb,$ and $\Sup\Bra{a_1,a_2,\dots}=\sqrt{2}.$}
...
\ChDecl{Ch0D}{0$\cdot$D}{}

\vspace{4pt}

%\Anchor{0D12}\ProblemN{12}{
%	\TextA{Supp $a_1,a_2,\dots\in\RbbP{n}$ convg of lim $L.$ \,Prove $F=\Bra{L,a_1,a_2,\dots}$ is closed.}
%}We show $\RbbP{n}\Backslash F$ is open. Asum $\exists\,b\in\RbbP{n}\Backslash F$ suth $\forall\delta>0,\,B\Par{b,\delta}\not\subseteq\RbbP{n}\Backslash F\Longleftrightarrow\exists\,x\in B\Par{b,\delta}\cap F.$\parSol{}
...
\Anchor{0D15}\ProblemN{15}{
	\TextA{Supp $F$ is a closed subset of $\Rbb.$ Prove $\Bra{a^2:a\in F}$ is closed.}
}
\SepLine
...
By [0.59], $G=\Interval{(}{)}{{-\infty,-n}}\cup\Interval{(}{)}{{n,\infty}}\cup I_1\cup I_2\cup\cdots$ \Par{disjoint} $\Longrightarrow I_1\cup I_2\cup\cdots=\Rbb\Backslash F.$\PfEnd
\SepLine

\Anchor{0D20}\ProblemN{20}{
	\TextA{Prove $\forall b\in\RbbP{n},\,\delta>0,\,\Bra{a\in\RbbP{n}:\Dvert{a-b}{_\infty}>\delta},\Bra{a\in\RbbP{n}:\Dvert{a-b}>\delta}$ are open.}
}Asum $\exists\,b\in\RbbP{n},\delta>0$ suth $\Bra{a\in\RbbP{n}:\Dvert{a-b}{_\infty}\leqslant\delta}$ is not closed.\parSol{}
...
\Anchor{0D23}\ProblemN{23}{
	\TextA{Supp $F_{\!1},F_{\!2}$ are disj closed subsets of $\Rbb$ suth $F_{\!1}\cup F_{\!2}$ is interval. Prove $F_{\!1}=\emptySet$ or $F_{\!2}=\emptySet.$}
}Asum both $F_{\!1},F_{\!2}\neq\emptySet\Rightarrow F_{\!1}\cup F_{\!2}$ contains at least two elem $\Rightarrow$ infily many.\parSol{}
Then \,$x\in F_{\!1}\cup F_{\!2}\Longleftrightarrow x\in F_{\!1}$ xor $x\in F_{\!2}.$ Thus $\exists\,x,\,\forall\delta>0,\,B\Par{x,\delta}\not\subseteq F_{\!1}\cup F_{\!2}.$\parSol{}
While $\forall x\in F_{\!1}\cup F_{\!2}=\Interval{[}{]}{a,b},$ we have $x\in\Interval{(}{)}{a,b}\Longleftrightarrow\exists\,\delta>0,\,B\Par{x,\delta}\subseteq\Interval{[}{]}{a,b}.$ Ctradic.\PfEnd
\SepLine
...
}Consider a seq disti $a_1,b_1,a_2,b_2,\dots\in\Rbb$ where each $a_j\in\Rbb\Backslash\Qbb$ and $\Bra{b_1,b_2,\dots}=\Qbb.$\parSol{}
Define \,$f\Par{a_j}=b_{2j+1},\:f\Par{b_k}=b_{2k}\Rightarrow\,f^{-1}\Par{b_i}=\MathLeftBrace{l}{\!a_{\SmallPar{i-1}/2},\,\,\text{if }i\text{ is odd,}\\\!b_{i/2},\hfill\text{if }i\text{ is even.}}$\PfEnd\vspace{4pt}
\ANote $\nexists$ a seq of {\tgsl all} irrational numbers, but whenever use \,$f$, the input in $\Rbb\Backslash\Qbb$ are always countable.
\SepLine

\Anchor{0D26}\ProblemN{26}{
	\TextA{Supp $E,G\subseteq\RbbP{n},$ and $G$ is open. Prove $E+G=\Bra{x+y:x\in E,\:\!y\in G}$ is open.}
}Asum $E+G$ not open $\Longleftrightarrow\RbbP{n}\Backslash\Par{E+G}$ not closed.\vspace{2pt}\parSol{}
Then $\exists\,a=x+y\in E+G$ suth $\forall\delta>0,\exists\,b\notin E+G$ suth $\Dvert{a-b}{_\infty}<\delta.$\parSol{}
Let \:\!$z=b-x\notin G\Rightarrow\Dvert{y-z}{_\infty}<\delta\Rightarrow z\in B\Par{y,\delta}\subseteq G,\,\exists\,\delta>0.$\PfEnd\vspace{2pt}\parSol{}
\Or $\exists\,a_1,a_2,\dots\notin E+G$ while its lim $L=e+g\in E+G.$ \,又 $\forall x\in E,\,a_k-x\notin G$\parSol{}
$\Rightarrow\lim_{k\rightarrow\infty}\!\Par{a_k-e}=L-e=g\in G.$ \,Thus $\RbbP{n}\Backslash G$ not closed $\Longleftrightarrow G$ not open.\PfEnd
\SepLine
...
\ProblemB{
	\TextB{Supp $b\in A\subseteq\RbbP{m},\,f:A\rightarrow\RbbP{n}.$ Prove}
