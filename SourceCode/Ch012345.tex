...
If $C,D$ not posi while $-C,-D$ not posi $\Rightarrow C=D=\tilde{0},$ then with the asum $\tilde{0}\tilde{0}=\tilde{0},$ \,it still holds.\vspace{2pt}\par
%\BulletPointX For $C=D=\tilde{0},$ \,define $CD=\tilde{0}.$
...
\Blind{$CD$}${}=\Bra{a:a<ru,\:\forall r\in C^-,\,0\leqslant u\leqslant s,\,\forall s\notin D}.$\vspace{3pt}\par
\BulletPointX Note the `$0\leqslant u$'. Becs $C^-\neq\emptySet\Rightarrow 0\in C^-.$ If it is to be exactly $a<0$ in $LHS$ $\Par{{=\tilde{0}}}$, \,then $C^-=\zeroSubs,$\TextB{}
for if not, $\exists\,u=s>0,$ and $\exists\,r\in C^-\nonzero,$ such that $\exists\,a<ru<0.$ Hence `$0\leqslant u$' is actually `$0<u$'.\vspace{3pt}\par
...
`$a<ru$' cannot be changed to `$a\leqslant ru$' with ``$0<u<s,\,\forall s\notin D$''?????\vspace{2pt}\par
\BulletPointX Let ${}^+D=\Bra{u:0<u\leqslant s,\,\forall s\notin D}\supsetneq D^+.$
\SepLine

\BulletPointX Consider $-C$ and $D$ both posi. \;Omitting $C=\tilde{0},$ \,becs $C$ not posi $\notRightarrow{-C}$ posi.\vspace{1pt}\TextB{}
$CD=-\Sbra{\Par{{-C}}D}=-\Bra{a:a\leqslant cd,\,c\in\Par{{-C}}{^+},\,d\in D^+}$\TextB{}
\Blind{$CD$}${}=\Bra{{a}:a$\uline{${}<-b<{}$}$-cd,\,\forall d\in D^+,\,\forall c\text{\;suth\;}$\uline{$0<c<-c',\,\exists\,c'$}${}\not\in C}$\hfill{\FontSmall\BigPar{$0<{}$\uline{$cd<-c'd$}}}\vspace{4pt}\TextB{}
...
$\Blind{CD}=\Bra{a:a\leqslant c'd,\;$\uline{$\forall c'\in C^-,$}$\,\forall d\in D^+}=\Bra{a:a\leqslant rd,\;\forall r\in C^-,\,\forall d\in D^+}.$
%\TextB{\vspace{-20pt}}\hspace{310pt}\begin{tikzpicture}[>=stealth, every node/.style={font=\scriptsize},scale=1.4]
...
\BulletPointX Let $LHS=\Bra{cs:c\in C,\,s\notin D},\;RHS=\Bra{a:a\leqslant rd,\,\forall r\in C^-,\,\forall d\in D^+}.$\TextB{}
Supp $a\leqslant rd.$ Let $b\notin D\Rightarrow d<b.$ Let $q=b\big/d.$\TextB{}
$r'=rq\Rightarrow r'd=rb.$\TextB{}
Supp $c,s$ suth $cs\in LHS.$ \,Then $cs<rs,\,\forall r\in C^-.$
\SepLine
