...
\BulletPointX Note that $\Bra{u:0<u\leqslant s,\,\forall s\notin D}\supsetneq D^+.$ We show ``$0<u\leqslant s,\,\forall s\notin D$'' is equiv to ``$\forall u\in D^+$''.\TextB{}
$LHS=\Bra{a:a<ru,\:\forall r\in C^-,\,0<u\leqslant s,\,\forall s\notin D},\;\Bra{a:a<rd,\:\forall r\in C^-,\,\forall d\in D^+}=RHS.$\TextB{}
Immed, $LHS\subseteq RHS.$ \,For $RHS\subseteq LHS$\,:\TextB{}
(I) If we cannot fix ``$\min\Qbb\Backslash D$'' by a certain scalar $s_M,$ then becs `$u\leqslant s$' is actually `$u<s$',\TextB{}
\HI $u\in\Qbp,\,u\not\in D^+\Longleftrightarrow u\geqslant s,\,\exists\,s\notin D\Longleftrightarrow u\notin\Bra{u:0<u<s,\,\forall s\notin D}.$\vspace{2pt}\TextB{}
(II) Othws, \,$s_M=\max D.$ \,Now in $LHS$, ``$a<rs_M,\,\forall r\in C^-$''.\TextB{}
\HII Supp \,$0<d_1<\cdots<d_n<\cdots\in D^+$ \Sbra{$d_n<s_M$} \,and $0>r_{\:\!\!1}>\cdots>r_{\:\!\!m}>\cdots\in C^-.$\TextB{}
%\NOTICE that $d_{\infty}$ is an ordered pair $\Par{d_n,d_{n+1}}$ with $d_n<d_{n+1}.$ Simlr, $\Par{r_m,r_{m+1}}$ with $r_{m+1}<r_m.$\TextB{}
\HII %We show ``$a<r_{\:\!\!m}\,d_{n}$'' is equiv to ``$a<r_{\:\!\!m}\,s_M$''. 
Becs \;\!$r_{\:\!\!m+1}\,s_M<r_{\:\!\!m}\,s_M<r_{\:\!\!m}\,d_{n+1}<r_{\:\!\!m}\,d_n>r_{\:\!\!m+1}\,d_{n+1}.$ Now $r_{\:\!\!m+1}\,s_M<r_{\:\!\!m}\,s_M\notin CD=RHS.$\TextB{}
\HII Asum $\exists\,t\in RHS\Backslash LHS\Rightarrow{}$\uline{$r_{\:\!\!m}\,s_M<t$}${}<r_{\:\!\!m}\,d_n,\,\forall m,n\in\Nbp.$ \,Let $u=t\big/r_{\:\!\!m}\Rightarrow s_M>u>d_n.$ Ctradic.
\PfEnd
\SepLine

\BulletPointX For $C$ posi and $D$ not posi. Consider $C$ and $-D$ both posi. \;Omitting $D=\tilde{0}.$\vspace{1pt}\TextB{}
...
$\Blind{CD}=\Bra{a:a<ct,\,\forall c\in C^+,\,\forall t\text{\;suth\;}s<t<0,\,\exists\,s\in D^-}$\TextB{}
$\Blind{CD}=\Bra{a:a<cs,\,\forall c\in C^+,\,\forall s\in D^-\nonzero}.$
\SepLine

\BulletPointX For $C$ not posi and $D$ posi, we show $\Bra{a:a<rd,\:\forall r\in C^-,\,\forall d\in D^+}=\Bra{cs:c\in C,\,s\notin D}.$\TextB{}
$\forall s\notin D,\forall d\in D^+,\,d<s\Rightarrow\forall c\in C,\forall r\in C^-,\,rd>cd>cs,$ \Or $rd>rs>cs.$ \:Thus $RHS\subseteq LHS.$\vspace{2pt}\TextB{}
Supp $a\in LHS\Rightarrow a\big/d\in C,\,\forall d\in D^+.$ \,We show $\exists\,\varepsilon>0$ suth {\Large$\frac{a}{\:d\,+\,\varepsilon\:}$}${}\in C,\,\forall d\in D^+.$ So that $a\in RHS.$\vspace{2pt}\TextB{}
Asum $\forall\varepsilon>0,\,\,${\Large$\frac{a}{\:d'\,+\,\varepsilon}$}${}\notin C\,\Sbra{\!=r_{\:\!\!0}\in C^-},\,\exists\,d'\in D^+.$ Then $\exists\,r_{\:\!\!1}\in C^-,\,r_{\:\!\!1}>r_{\:\!\!0}\Rightarrow a>r_{\:\!\!1}\:\!d'+\varepsilon\:\!d'.$\vspace{2pt}\TextB{}
Now $\forall r\in C^-,\forall d\in D^+,\;r\:\!d>r_{\:\!\!1}\:\!d'+\varepsilon\:\!d'.$ 又 $r_{\:\!\!1}\:\!d'>r\:\!d'>r\:\!d$ \,and\, $\varepsilon\:\!d'>0.$ Ctradic.
%Rewrite $a\big/d<r,\,\forall r\in C^-,\,d\in D^+\Longleftrightarrow a\big/d\in C,\,\forall d\in D^+.$ 
%Asum $\exists\,a\in LHS$ suth $a\big/s\neq c,\,\forall c,s.$\TextB{}
%Note that $\forall s\notin D,d\in D^+,\:0<a\;\!\Par{1\big/s-1\big/d}\Rightarrow{}$\uline{$\forall\varepsilon>0,\,\exists\,s,d$ suth $\varepsilon=a\;\!\Par{1\big/s-1\big/d}.$}\TextB{}
%Thus for $a\in LHS,\,\exists\,s,d$ suth  $a\big/d+\varepsilon=a\big/s\in C.$
\SepLine

%\BulletPointX For $C$ posi and $D$ not posi, to make $CD=DC,$ \,we define\, $CD=\Bra{a:a<cs,\;\forall c\in C^+,\,s\in D^-}.$
%\SepLine

%\BulletPointX We show $\tilde{q}D=\Bra{a:a<qd,}$
...
\par\quad
(1) $\BigPar{\Par{BC}D}{^+}=\Bra{\Par{bc}d:bc\in\Par{BC}{^+},\,d\in D^+}=\Bra{b\Par{cd}:b\in B^+,\,cd\in\Par{CD}{^+}}=\BigPar{B\Par{CD}}{^+}.$\par\quad
(4) $\Par{BC}D=\Bra{a:a<rs,\:r\in\Par{BC}{^-},\,s\in D^-}.$\par\quad

\par\quad
$\BigPar{B\Par{C+D}}{^+}=\Bra{bs:b\in B^+,\,s\in\Par{C+D}{^+}}=\Bra{bc+bd:b\in B^+,\,-bc<bd,\,c\in C,\,d\in D}=P.$\par\quad
$\Bra{bc:b\in B^+,\,c\in C^+}+\Bra{b'd:b'\in B^+,\,d\in D^+}=\Bra{bc+b'd:b,b'\in B^+,\,c\in C^+,\,d\in D^+}=Q.$\par\quad

%\Blind{(1)} $\Bra{a:a\leqslant b\Par{c+d},\:b\in B^+,\,c+d\in\Par{C+D}{^+}}=\Bra{a:a\leqslant bc,\,b\in B^+,\,c\in C^+}$
\SepLine
